const heatmapData = [
  0, 1, 0, 2, 3, 1, 0, 1, 2, 2, 1, 0, 3, 3, 0, 0, 1, 2, 2, 1, 0, 1, 1, 3, 2, 1,
  1, 2, 2, 3, 3, 1, 0, 1, 2,
];
      ...
      <div className="flex-1">
        ...
        <div className="flex flex-col gap-3">
          {["インテリア", "スムーズ", "トラブル"].map((word) => (
            <div
              key={word}
              className="bg-white/5 border border-white/10 rounded-2xl p-4 flex justify-between items-center hover:bg-white/10 transition-colors cursor-pointer"
            >
              <span className="font-medium text-lg text-zinc-100">{word}</span>
              <span className="text-xs text-rose-400 font-mono bg-rose-500/10 px-2 py-1 rounded-md font-semibold">
                Fail: 5
              </span>
            </div>
          ))}
