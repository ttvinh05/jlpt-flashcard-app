...
const Header = ({ handleToggle }: HeaderProps) => {
  ...
  if (error) throw new Response("Đã có lỗi xảy ra", { status: 404 });
      ...
      {loading ? (
        ...
        <>
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon-xl" onClick={handleToggle}>
              <FiMenu />
            </Button>

            <Link
              to="/"
              className="hidden sm:block font-bold text-xl tracking-wider text-zinc-50 hover:opacity-80 transition-opacity"
            >
          ...
          <div className="flex-1 max-w-2xl px-4 md:px-8">
            <div className="relative group" ref={inputRef}>
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none z-10">
                <FiSearch
                  className="text-zinc-400 group-focus-within:text-blue-400 transition-colors"
                  size={18}
                />
              </div>
              <Input
                ...
                <div className="absolute top-12 left-0 w-full bg-zinc-900/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl shadow-black overflow-hidden z-50 py-2">
                  ...
                  <div className="max-h-[400px] overflow-y-auto scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
                    {filteredDecks.map((item) => (
                      <div
                        key={item.id}
                        className="flex items-center gap-3 px-4 py-3 hover:bg-white/5 cursor-pointer transition-colors group/item"
                      >
